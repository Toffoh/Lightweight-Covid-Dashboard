...
''' Create custom loggers so we can control what is being logged'''
    ...
    for u in update_scheduled_times:
        if u-time.time() <= 0:
            try:
                ...
                logr.exception('Index Error had occurred at {}'.format(datetime.now().strftime('%Y-%m-%d | %H:%M:%S')))
                continue
...
def log_requests(response):
    ...
    if response.status_code==500:
        logr.critical(str(log_structure))
        print('\nCritical Error - The server could not process that request\n')
    if response.status_code==404:
        logr.error(str(log_structure))
        print('\nError 404 - Requested resource not found... Try checking your URL\n')
    return response
